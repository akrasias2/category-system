object Version {
    const val KOTLIN_SERVER_TEMPLATE = "0.1.0"

    const val KOTLIN = "1.7.22"

    const val KOTEST = "5.5.4"
    const val KOTEST_ASSERTIONS_ARROW = "1.3.0"

    const val MOCK_K = "1.13.4"

    const val GRADLE_KTLINT = "10.2.1"
    const val GRADLE_SHADOW = "7.0.0"

    const val KOTLINX_COROUTINES = "1.6.4"
    const val KOTLINX_SERIALIZATION_JSON = "1.4.1"

    const val ARROW = "1.1.5"

    const val LOGBACK = "1.2.10"

    const val SPRING_BOOT = "2.7.8"
    const val SPRING_DEPENDENCY_MANAGEMENT = "1.1.0"
    const val SPRINGMOCK_K = "3.1.2"
    const val DOCKER_COMPOSE = "0.16.8"

    const val TYPESAFE_CONFIG = "1.4.2"

    const val SPRING_DATA_R2DBC = "1.4.2"
    const val R2DBC_BOM = "Arabba-SR12"
}
